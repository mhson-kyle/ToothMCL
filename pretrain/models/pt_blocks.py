...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_p = nn.Sequential(
            nn.Linear(3, 3), 
            nn.BatchNorm1d(3, momentum=0.1, eps=1e-5), 
            nn.ReLU(inplace=True), 
            nn.Linear(3, out_planes)
            )
        self.linear_w = nn.Sequential(
            nn.BatchNorm1d(mid_planes, momentum=0.1, eps=1e-5), 
            nn.ReLU(inplace=True),
            nn.Linear(mid_planes, mid_planes // share_planes),
            nn.BatchNorm1d(mid_planes // share_planes, momentum=0.1, eps=1e-5), 
            nn.ReLU(inplace=True),
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        # Process p_r through linear_p layers
        for i, layer in enumerate(self.linear_p):
            if i == 1:  # BatchNorm layer
                # Reshape for BatchNorm: (n, nsample, 3) -> (n*nsample, 3)
                p_r_reshaped = p_r.reshape(-1, 3)
                p_r_reshaped = layer(p_r_reshaped)
                # Reshape back: (n*nsample, 3) -> (n, nsample, 3)
                p_r = p_r_reshaped.reshape(p_r.shape[0], p_r.shape[1], 3)
            else:
                p_r = layer(p_r)
        
        w = x_k - x_q.unsqueeze(1) + p_r.view(p_r.shape[0], p_r.shape[1], self.out_planes // self.mid_planes, self.mid_planes).sum(2)
        
        # Process w through linear_w layers
        for i, layer in enumerate(self.linear_w):
            if i % 3 == 0:  # BatchNorm layers
                # Reshape for BatchNorm: (n, nsample, c) -> (n*nsample, c)
                w_reshaped = w.reshape(-1, w.size(-1))
                w_reshaped = layer(w_reshaped)
                # Reshape back: (n*nsample, c) -> (n, nsample, c)
                w = w_reshaped.reshape(w.shape[0], w.shape[1], -1)
            else:
                w = layer(w)
...
class PointTransformerBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16):
        ...
        # Use regular BatchNorm1d by default
        self.bn1 = nn.BatchNorm1d(planes, momentum=0.1, eps=1e-5)
        self.transformer2 = PointTransformerLayer(planes, planes, share_planes, nsample)
        self.bn2 = nn.BatchNorm1d(planes, momentum=0.1, eps=1e-5)
        self.linear3 = nn.Linear(planes, planes * self.expansion, bias=False)
        self.bn3 = nn.BatchNorm1d(planes * self.expansion, momentum=0.1, eps=1e-5)
        self.relu = nn.ReLU(inplace=True)
