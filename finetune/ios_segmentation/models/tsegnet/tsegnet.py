...
class TSegNetModule(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        moved_points = utils.torch_to_numpy(l3_xyz + offset_result).T.reshape(-1,3)
        moved_points = moved_points[utils.torch_to_numpy(dist_result).reshape(-1)<0.3,:]
        ...
        #utils.print_3d(utils.np_to_pcd(moved_points,color=[0,1,0]), center_points[0])
        ...
        nn_crop_indexes = ou.get_nearest_neighbor_idx(utils.torch_to_numpy(l0_xyz.permute(0,2,1)), center_points, 3072)
