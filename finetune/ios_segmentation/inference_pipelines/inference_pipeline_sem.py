...
class InferencePipeLine:
    ...
    def __call__(self, stl_path):
        DEBUG=False
        _, mesh = utils.read_txt_obj_ls(stl_path, return_mesh=True, use_tri_mesh=True) #TODO slow processing speed
        ...
        sampled_feats = utils.resample_pcd([vertices.copy()], 24000, "fps")[0] #TODO slow processing speed
        
        with torch.no_grad():
            input_cuda_feats = torch.from_numpy(np.array([sampled_feats.astype('float32')])).cuda().permute(0, 2, 1) # (B, N, C) -> (B, C, N)
